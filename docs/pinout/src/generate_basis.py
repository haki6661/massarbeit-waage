"""Erzeugt docs/pinout/massarbeit-basis-pinout.png aus dem offiziellen
...
for num, pt, color in pins:
    ...
    edge_x = cx + r if right_side else cx - r
    d.line([bx - br, by, edge_x, cy], fill=color, width=4)
...
footer = "Bildquelle: offizielles Pinmap-Bild github.com/Xinyuan-LilyGO/LilyGo-T-OI-PLUS (annotiert)"
fb = cd.textbbox((0, 0), footer, font=font_footer)
cd.text(((W - (fb[2] - fb[0])) / 2, fy), footer, font=font_footer, fill="#9aa5b1")

canvas.save("../massarbeit-basis-pinout.png")
print(canvas.size)
